        ...
        <tbody>
            @foreach ($users as $user)
                <tr class="deleted-row-{{ $user->id }}">
                    ...
                    <td class="pl-0 py-4">
                        <div class="symbol symbol-50 symbol-light mr-1">
                            <span class="symbol-label">
                                <img src="{{ $user->imgs ?? 'https://alshshamil.com/assets/arab.png' }}"
                    ...
                    <td>
                        <span
                            class="label label-lg label-light-{{ $user->span() }} label-inline">{{ $user->role() }}</span>
                    ...
                    <Td>
                        ...
                        <a href="{{ route('admin.UserEdit', $user->id) }}"
                            data-href="{{ route('admin.UserEdit', $user->id) }}" data-entity_id="{{ $user->id }}"
                        ...
                        <a href="javascript:;" data-href="{{ route('admin.UserDelete', $user->id) }}"
                            data-entity_id="{{ $user->id }}" data-token="{{ csrf_token() }}"
